...
const ThemeDecoration = ({ theme, reducedMotion }: { theme: string; reducedMotion: boolean }) => {
  switch (theme) {
    case 'noise':
      return null; // Handled by CSS class
    case 'lines':
      return (
        <svg
    ...
    case 'graduation':
      return (
        <motion.div
          className="absolute bottom-20 right-10 md:right-20 opacity-10"
    ...
    case 'timeline':
      return (
        <div className="absolute left-8 md:left-16 top-1/4 bottom-1/4 w-px bg-gradient-to-b from-transparent via-primary/30 to-transparent" />
      );
    case 'layers':
      return (
        <svg
          className="absolute right-0 top-1/2 -translate-y-1/2 w-48 md:w-64 opacity-20 pointer-events-none"
          viewBox="0 0 200 200"
        >
...
export function SlideSection({ slide, index }: SlideSectionProps) {
  ...
  const itemVariants = {
    ...
    visible: { 
      opacity: 1, 
      y: 0,
      transition: { duration: reducedMotion ? 0.1 : 0.6, ease: [0.25, 0.46, 0.45, 0.94] as const }
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0">
        {hasImage ? (
          <>
            {/* Image with fallback */}
            <div
              className="absolute inset-0 bg-cover bg-center slide-gradient"
              style={{
      ...
      {/* Content */}
      <motion.div
        className="relative z-10 container mx-auto px-6 md:px-12 lg:px-20 py-20 max-w-4xl"
      ...
      >
        ...
        <motion.h2
          ...
          variants={itemVariants}
          className="text-display font-black text-foreground mb-12 leading-tight"
        >
          {slide.title}
        </motion.h2>
        ...
        >
          <p className="stat-label">{slide.statLabel}</p>
          <p className="text-stat stat-value">{slide.statValue}</p>
        ...
        <motion.p 
          variants={itemVariants}
          className="text-xl md:text-2xl font-medium text-foreground italic"
        >
          "{slide.question}"
        </motion.p>
      </motion.div>
